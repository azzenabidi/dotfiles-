...
BarWidget {
  ...
  // When no real keyboard holds the main flag (fcitx5 does here), fall back to
  // the physical keyboard so the label never vanishes.
  function selectKeyboard(keyboards) {
    const typed = keyboards.filter(k => !String(k.name).startsWith("hl-virtual-keyboard"))
    const physical = typed.find(k => String(k.name).toLowerCase().indexOf("translated") !== -1) ?? typed[0]
    return typed.find(k => k.main) ?? typed.find(k => k.name === root.keyboardName) ?? physical
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        root.layoutLabel = kb.active_keymap.split(/\s+/)[0].substring(0, 2).toUpperCase()
      }
    }
  }
